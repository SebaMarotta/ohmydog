...
public interface CampanaDonacionRepository  extends JpaRepository<CampanaDonacion, Long> {
    @Query("SELECT c FROM CampanaDonacion c WHERE c.fechaVencimiento = :fechaVencimiento and c.activa = true")
    public List<CampanaDonacion> findByDateAndVisible(@Param("fechaVencimiento") LocalDate fecha);
